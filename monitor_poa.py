...
def buscar_edicoes_novas():
    ...
    # Busca todos os links de PDF com numero de edicao no texto adjacente
    # Padrao: href="...edicao-NNNN-...pdf"
    padrao_url = re.compile(r'href="(https://poa\.sp\.gov\.br/wp-content/uploads/[^"]+\.pdf)"')
    padrao_num = re.compile(r'edicao-(\d+)-')

    novas = []
    vistos = set()

    for match in padrao_url.finditer(texto):
        url_pdf = match.group(1)
        num_match = padrao_num.search(url_pdf)
        if num_match:
